...
export const transformTicketmasterEvent = (event: TicketmasterEvent): LocalEvent => {
...
export class EventsService {
  ...
  static async getEventsByCity(params: SearchParams): Promise<LocalEvent[]> {
    try {
      ...
      const response = await ticketmasterApi.get<TicketmasterResponse>('/events.json', {
  ...
  static async getEventById(eventId: string): Promise<LocalEvent | null> {
    try {
      const response = await ticketmasterApi.get<TicketmasterEvent>(`/events/${eventId}.json`);
