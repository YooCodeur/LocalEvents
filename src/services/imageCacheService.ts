...
export class ImageCacheService {
  ...
  static async isImageCached(imageUrl: string): Promise<string | null> {
    try {
      ...
      const now = Date.now();
      const isExpired = (now - imageMetadata.timestamp) > IMAGE_CACHE_CONFIG.IMAGE_TTL;
      ...
      console.error('❌ Erreur lors de la vérification du cache d\'image:', error);
      return null;
    }
  }

  // Télécharger et mettre en cache une image
  static async cacheImage(imageUrl: string, eventId: string): Promise<string | null> {
    try {
      const imageKey = this.generateImageKey(imageUrl);
      const localPath = this.getLocalImagePath(imageKey);
      ...
      console.log(`✅ Image mise en cache: ${imageKey} (${Math.round(fileSize / 1024)}KB)`);
      return localPath;
    } catch (error) {
  ...
  static async cacheEventsImages(events: LocalEvent[]): Promise<void> {
    try {
      ...
      console.error('❌ Erreur lors de la mise en cache des images d\'événements:', error);
    }
  }
  ...
  static async getImageMetadata(): Promise<ImageCacheMetadata> {
    try {
      const metadataString = await AsyncStorage.getItem(IMAGE_CACHE_CONFIG.METADATA_KEY);
      if (!metadataString) {
        return {
          images: {},
          totalSize: 0,
      ...
      console.error('❌ Erreur lors de la lecture des métadonnées d\'images:', error);
      return {
        images: {},
        totalSize: 0,
        lastCleanup: Date.now(),
  ...
  static async updateImageMetadata(imageKey: string, imageMetadata: CachedImageMetadata): Promise<void> {
    try {
      const metadata = await this.getImageMetadata();
      ...
      if (oldImageData) {
        metadata.totalSize = metadata.totalSize - oldImageData.fileSize + imageMetadata.fileSize;
      } else {
      ...
      await AsyncStorage.setItem(IMAGE_CACHE_CONFIG.METADATA_KEY, JSON.stringify(metadata));
    } catch (error) {
      console.error('❌ Erreur lors de la mise à jour des métadonnées d\'image:', error);
    }
  }
  ...
  static async removeImageFromCache(imageKey: string): Promise<void> {
    try {
      ...
      if (imageMetadata) {
        ...
        await FileSystem.deleteAsync(imageMetadata.localPath, { idempotent: true });
    ...
    try {
      ...
      if (imageKeys.length > IMAGE_CACHE_CONFIG.MAX_IMAGES) {
        ...
        const sortedKeys = imageKeys.sort((a, b) => 
          metadata.images[a].timestamp - metadata.images[b].timestamp
        );
        
        const toRemove = sortedKeys.slice(0, imageKeys.length - IMAGE_CACHE_CONFIG.MAX_IMAGES);
      ...
      if (metadata.totalSize > IMAGE_CACHE_CONFIG.MAX_CACHE_SIZE) {
        const sortedKeys = Object.keys(metadata.images).sort((a, b) => 
          metadata.images[a].timestamp - metadata.images[b].timestamp
        );
      ...
      console.error('❌ Erreur lors de l\'application des limites de cache:', error);
    }
  }
  ...
  static async cleanupExpiredImages(): Promise<void> {
    try {
      ...
      for (const [key, imageData] of Object.entries(metadata.images)) {
        const isExpired = (now - imageData.timestamp) > IMAGE_CACHE_CONFIG.IMAGE_TTL;
        if (isExpired) {
      ...
      await AsyncStorage.setItem(IMAGE_CACHE_CONFIG.METADATA_KEY, JSON.stringify(updatedMetadata));

      console.log(`✨ Nettoyage des images terminé: ${expiredKeys.length} images supprimées`);
    } catch (error) {
  ...
  static async clearAllImages(): Promise<void> {
    try {
      ...
      const dirInfo = await FileSystem.getInfoAsync(IMAGE_CACHE_CONFIG.CACHE_DIR);
      if (dirInfo.exists) {
        await FileSystem.deleteAsync(IMAGE_CACHE_CONFIG.CACHE_DIR, { idempotent: true });
      }
      ...
      await AsyncStorage.setItem(IMAGE_CACHE_CONFIG.METADATA_KEY, JSON.stringify(emptyMetadata));
  ...
  static async getCacheStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        totalSizeMB: Math.round((metadata.totalSize / (1024 * 1024)) * 100) / 100,
